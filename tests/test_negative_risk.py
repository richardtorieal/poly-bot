...
@pytest.mark.asyncio
@respx.mock
async def test_negative_risk_scan_no_arb(mock_settings):
    # Mock market with 2 tokens
    respx.get("https://clob.polymarket.com/markets").mock(
        return_value=httpx.Response(200, json={
            "markets": [{"id": "m1", "question": "Test?", "tokens": [{"token_id": "t1"}, {"token_id": "t2"}]}]
        })
    )
    ...
    async with PolymarketClient(mock_settings) as client:
        strategy = NegativeRiskStrategy(client, profit_margin=0.02, polling_interval=0)
        await strategy.run_iteration()

@pytest.mark.asyncio
@respx.mock
async def test_negative_risk_scan_with_arb(mock_settings, caplog):
    # Mock market
    respx.get("https://clob.polymarket.com/markets").mock(
        return_value=httpx.Response(200, json={
            "markets": [{"id": "m1", "question": "Test?", "tokens": [{"token_id": "t1"}, {"token_id": "t2"}]}]
