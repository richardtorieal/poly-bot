...
def objective(trial):
    ...
    # 4. Lookback minutes [2, 6]
    lookback_minutes = trial.suggest_int('lookback_minutes', 2, 6)
    ...
    er_lookback = trial.suggest_int('er_lookback', 2, 15)
...
def main():
    logger.info("Initializing multi-process Optuna parameter sweep...")
    study_name = "btc_trend_opt_mp"
    storage_url = "sqlite:///optuna_study_mp.db"
    
    if os.path.exists("optuna_study_mp.db"):
        try:
            os.remove("optuna_study_mp.db")
