...
def objective(trial):
    ...
    btc_threshold = trial.suggest_float('btc_threshold', 0.00005, 0.00030)
    ...
    # Enforce btc_threshold_down >= 0.00005 and within 10% of btc_threshold_up
...
def main():
    logger.info("Initializing Volatility adapt In-Sample Optuna sweep...")
    study_name = "btc_trend_opt_vol"
    storage_url = "sqlite:///optuna_study_vol.db"
    
    if os.path.exists("optuna_study_vol.db"):
        try:
            os.remove("optuna_study_vol.db")
    ...
    # Enqueue baseline parameters (the current main parameters with volatility_adapt: true)
    baseline_params_vol = {
        'lookback_minutes': 2,
        'er_lookback': 2,
    ...
    study.enqueue_trial(baseline_params_vol)
    
    # Enqueue baseline parameters (the current main parameters with volatility_adapt: false)
    ...
    study.enqueue_trial(baseline_params_no_vol)
    
    logger.info("Evaluating enqueued baseline trials sequentially first...")
