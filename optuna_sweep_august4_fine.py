...
def objective(trial):
    ...
    # 2. Local search space with strict constraints enforced in the distribution
    btc_threshold = trial.suggest_float('btc_threshold', 0.00012, 0.00015)
    
    # Enforce btc_threshold_up >= 0.00005 and within 9% of btc_threshold to ensure ratio <= 1.10
    btc_threshold_up = trial.suggest_float('btc_threshold_up', max(0.00005, 0.91 * btc_threshold), min(0.00025, 1.09 * btc_threshold))
    
    # Enforce btc_threshold_down >= 0.00005 and within 9% of btc_threshold_up
    low_down = max(0.00005, 0.91 * btc_threshold_up)
    high_down = min(0.00025, 1.09 * btc_threshold_up)
    btc_threshold_down = trial.suggest_float('btc_threshold_down', low_down, high_down)
    
    # er_threshold >= 0.50
    er_threshold = trial.suggest_float('er_threshold', 0.75, 0.95)
...
def main():
    logger.info("Initializing Fine-Tuned August 4 In-Sample optimized Optuna sweep...")
    study_name = "btc_trend_opt_august4_fine"
    storage_url = "sqlite:///optuna_study_august4_fine.db"
    
    if os.path.exists("optuna_study_august4_fine.db"):
        try:
            os.remove("optuna_study_august4_fine.db")
