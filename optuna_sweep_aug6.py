...
def objective(trial):
    ...
    er_lookback = trial.suggest_int('er_lookback', lookback_minutes, lookback_minutes + 4)
    
    # 2. Threshold search space with strict constraints
    btc_threshold = trial.suggest_float('btc_threshold', 0.00010, 0.00028)
    ...
    exit_profit_pct = trial.suggest_float('exit_profit_pct', 0.010, 0.030)
    
    # Stop loss >= 1.5% (0.015)
    stop_loss_pct = trial.suggest_float('stop_loss_pct', 0.015, 0.050)
    
    max_minutes_elapsed = trial.suggest_float('max_minutes_elapsed', 8.5, 13.0)
    
    # EMA trend filter
    use_ema_filter = trial.suggest_categorical('use_ema_filter', [False])
...
def main():
    logger.info("Initializing In-Sample Optuna sweep for August 6...")
    study_name = "btc_trend_opt_aug6"
    storage_url = "sqlite:///optuna_study_aug6.db"
    
    if os.path.exists("optuna_study_aug6.db"):
        try:
            os.remove("optuna_study_aug6.db")
    ...
    # Enqueue baseline parameters (Top 3 trials from Aug 5)
    baseline_params_1 = {
        'lookback_minutes': 2,
        'er_lookback': 2,
    ...
    baseline_params_2 = {
        'lookback_minutes': 2,
        'er_lookback': 2,
        'btc_threshold': 0.00018741160825287104,
        'btc_threshold_up': 0.00018736203202304272,
        'btc_threshold_down': 0.0001714176080972968,
        'er_threshold': 0.6214221166647838,
        'exit_profit_pct': 0.01053272791625055,
        'stop_loss_pct': 0.018210026630421965,
        'max_minutes_elapsed': 10.524027848744007,
        'use_ema_filter': False,
        'ema_span': 39,
        'volatility_adapt': True,
        'use_trailing_stop': True,
        'trailing_stop_activation_pct': 0.005216452338294055,
        'trailing_stop_drop_pct': 0.0035745523831074446
    }
    
    baseline_params_3 = {
        'lookback_minutes': 2,
        'er_lookback': 2,
        'btc_threshold': 0.0001954561705548809,
        'btc_threshold_up': 0.00018758254163485626,
        'btc_threshold_down': 0.00017267556662896143,
        'er_threshold': 0.6049892703954185,
        'exit_profit_pct': 0.01003233881594802,
        'stop_loss_pct': 0.01804558160345708,
        'max_minutes_elapsed': 10.53340347427362,
        'use_ema_filter': False,
        'ema_span': 44,
        'volatility_adapt': True,
        'use_trailing_stop': True,
        'trailing_stop_activation_pct': 0.005255773351416482,
        'trailing_stop_drop_pct': 0.00295390157509709
    }
    
    study.enqueue_trial(baseline_params_1)
    study.enqueue_trial(baseline_params_2)
    study.enqueue_trial(baseline_params_3)
    
    logger.info("Evaluating enqueued baseline trials sequentially...")
    study.optimize(objective, n_trials=3)
    
    num_workers = 6
    trials_per_worker = 100 # 600 trials total
    
    logger.info(f"Spawning {num_workers} parallel workers to run {trials_per_worker} trials each (total {num_workers * trials_per_worker + 3} trials)...")
