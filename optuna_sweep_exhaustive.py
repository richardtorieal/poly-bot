...
# Load dataset ONCE on module import (per process)
...
def objective(trial):
    ...
    btc_threshold = trial.suggest_float('btc_threshold', 0.000075, 0.000105)
    
    # Symmetric/near-symmetric triggers (must be within 10% of each other)
    ...
    # Minimum er_threshold >= 0.5
    er_threshold = trial.suggest_float('er_threshold', 0.50, 0.60)
    
    # Tradeability constraints: profit target >= 1.0%, stop loss >= 1.5%
    exit_profit_pct = trial.suggest_float('exit_profit_pct', 0.011, 0.016)
    stop_loss_pct = trial.suggest_float('stop_loss_pct', 0.040, 0.055)
    
    # Max minutes elapsed
    max_minutes_elapsed = trial.suggest_float('max_minutes_elapsed', 10.0, 11.2)
    
    # Trailing stop parameters (activation must be less than exit profit target)
    trailing_stop_activation_pct = trial.suggest_float('trailing_stop_activation_pct', 0.002, exit_profit_pct)
...
def run_worker(study_name, storage, n_trials):
    ...
    time.sleep(np.random.rand() * 2) # Stagger worker start
    study.optimize(objective, n_trials=n_trials)

def main():
    logger.info("Initializing exhaustive fine-tuning Optuna sweep (with parameterized trailing stops)...")
    ...
    baseline_params = {
        ...
        'trailing_stop_activation_pct': 0.0135,
        'trailing_stop_drop_pct': 0.0135
    }
    study.enqueue_trial(baseline_params)
    
    # Run enqueued baseline trial first sequentially
