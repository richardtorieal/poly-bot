...
def objective(trial):
    ...
    # 1. Lookback window
    lookback_minutes = trial.suggest_int('lookback_minutes', 2, 5)
    ...
    btc_threshold = trial.suggest_float('btc_threshold', 0.00006, 0.00025)
    ...
    ema_span = trial.suggest_int('ema_span', 10, 90)
...
def main():
    logger.info("Initializing In-Sample optimized Optuna sweep (July 17 run)...")
    study_name = "btc_trend_opt_july17"
    storage_url = "sqlite:///optuna_study_july17.db"
    
    if os.path.exists("optuna_study_july17.db"):
        try:
            os.remove("optuna_study_july17.db")
    ...
    # Enqueue baseline parameters (July 16 parameters)
    baseline_params = {
        'lookback_minutes': 2,
        'er_lookback': 2,
