...
3. **BTC Price Direction (Sniper & Scalper)**:
   ...
   - **BTCTrendStrategy Parameters**: Optimizes lookback window (2m), ER threshold (0.5), position size (3%), exit profit (0.5%), stop loss (1.1%), and a time-elapsed prediction window filter (max_minutes_elapsed = 8.0) to maximize Sharpe (OOS 164.47) and reduce drawdown (OOS -3.25%).
