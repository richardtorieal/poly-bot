...
def objective(trial):
    ...
    # Suggest parameters within the strict quant mandate constraints
    # 1. Minimum entry threshold >= 0.00005
    btc_threshold = trial.suggest_float('btc_threshold', 0.00005, 0.0004)
    
    # 2. Symmetric/near-symmetric triggers (must be within 10% of each other)
    # Enforce minimum threshold limit of 0.00005 for both
    ...
    # 3. Minimum er_threshold >= 0.5
    er_threshold = trial.suggest_float('er_threshold', 0.5, 0.98)
    
    # 4. Lookback minutes [2, 8]
    lookback_minutes = trial.suggest_int('lookback_minutes', 2, 8)
    
    # 5. Tradeability constraints: profit target >= 1.0%, stop loss >= 1.5%
    exit_profit_pct = trial.suggest_float('exit_profit_pct', 0.010, 0.040)
    stop_loss_pct = trial.suggest_float('stop_loss_pct', 0.015, 0.060)
    
    # 6. Max minutes elapsed
    max_minutes_elapsed = trial.suggest_float('max_minutes_elapsed', 4.0, 14.0)
    
    # Position size is kept constant at 0.03 as per baseline
    pos_size_pct = 0.03
...
def main():
    logger.info("Starting Optuna parameter sweep...")
    ...
    for t in trials:
        ...
        oos_sharpe = t.user_attrs.get('oos_sharpe', 0.0)
        is_sharpe = t.user_attrs.get('is_sharpe', 0.0)
        
        # We need Max Drawdown strictly better than -30%
        if oos_max_dd > -0.30:
